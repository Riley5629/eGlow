...
public class PacketPlayOutChat extends PacketPlayOut {
    ...
    public Object toNMS(ProtocolVersion clientVersion) throws Exception {
        ...
        if (nms.minorVersion >= 16) {
            return nms.newPacketPlayOutChat.newInstance(component, nms.chatMessageTypeValues[this.getType().ordinal()], UUID.randomUUID());
        }
        if (nms.minorVersion >= 12) {
            return nms.newPacketPlayOutChat.newInstance(component, Enum.valueOf((Class<Enum>) nms.chatMessageTypeClass, this.getType().toString()));
        }
