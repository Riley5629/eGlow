				...
				public void write(ChannelHandlerContext context, Object packet, ChannelPromise channelPromise) throws Exception {
					...
					if (NMSHook.nms.packetPlayOutEntityMetadataClass.isInstance(packet)) {
						...
						if (glowingEntities.containsKey(entityId)) {
							...
							GlowVisibility gv = glowPlayer.getGlowVisibility();
							...
							if (gv.equals(GlowVisibility.NONE) || //Player can't see the glow or set to none
									(gv.equals(GlowVisibility.OTHER) && glowingTarget.getPlayer().equals(glowPlayer.getPlayer())) || //if glow is set to other
									(gv.equals(GlowVisibility.OWN) && !glowingTarget.getPlayer().equals(glowPlayer.getPlayer()))) { //if glow is set to own
								packetPlayOutEntityMetadata = new PacketPlayOutEntityMetadata(entityId, NMSHook.setGlowFlag(glowingTarget.getEntity(), false));
								super.write(context, packetPlayOutEntityMetadata.toNMS(glowPlayer.getVersion()), channelPromise);
								return;
							}
