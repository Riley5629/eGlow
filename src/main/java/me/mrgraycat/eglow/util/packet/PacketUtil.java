...
public class PacketUtil {
	...
	public void updatePlayer(IEGlowPlayer ePlayer) {
		...
		DataManager.getGlowPlayers().stream()
				...
				.forEach(glowPlayer -> {
					if (sendPackets && MainConfig.ADVANCED_TEAMS_SEND_PACKETS.getBoolean() && tabEnabled && !tabAddon.blockEGlowPackets()) {
						sendPacketNms(ePlayer, new PacketPlayOutScoreboardTeam(glowPlayer.getTeamName(),
								...
								MainConfig.ADVANCED_TEAMS_NAMETAG_VISIBILITY.getBoolean() ? "always" : "never",
								MainConfig.ADVANCED_TEAMS_ENTITY_COLLISION.getBoolean() ? "always" : "never",
					...
					if (!ePlayer.getGlowVisibility().equals(GlowVisibility.UNSUPPORTEDCLIENT)) {
						...
						boolean targetAll = glowPlayer.getGlowTarget().equals(GlowTargetMode.ALL);
						boolean custom = glowPlayer.getGlowTarget().equals(GlowTargetMode.CUSTOM) &&
								glowPlayer.getCustomTargetList().contains(ePlayer.getPlayer());
						boolean glowForAllOrOther = ePlayer.getGlowVisibility().equals(GlowVisibility.ALL) ||
								ePlayer.getGlowVisibility().equals(GlowVisibility.OTHER);
	...
	public synchronized void scoreboardPacket(IEGlowPlayer to, boolean join) {
		...
		DataManager.getGlowPlayers().stream()
				...
				.forEach(glowPlayer -> {
					...
					String nameTagVisibility = MainConfig.ADVANCED_TEAMS_NAMETAG_VISIBILITY.getBoolean() ? "always" : "never";
	...
	public void updateGlowing(IEGlowPlayer entity, boolean status) {
		...
		(entity.getGlowTarget().equals(GlowTargetMode.ALL) ? Bukkit.getOnlinePlayers() : entity.getCustomTargetList()).stream()
	...
	public void updateGlowTarget(IEGlowPlayer ePlayer) {
		...
		try {
			packetPlayOutEntityMetadata = new PacketPlayOutEntityMetadata(glowingEntityID,
					NMSHook.setGlowFlag(glowingEntity, ePlayer.getGlowTarget().equals(GlowTargetMode.ALL) && ePlayer.getGlowStatus()));
		} catch (Exception exc) {
			exc.printStackTrace();
		}
		...
		switch (ePlayer.getGlowTarget()) {
			...
			case CUSTOM:
				players.stream()
						...
						.filter(glowPlayer -> glowPlayer.getGlowVisibility().equals(GlowVisibility.ALL) ||
								(glowPlayer.getGlowVisibility().equals(GlowVisibility.OTHER) && !glowPlayer.getPlayer().equals(ePlayer.getPlayer()))
								|| (glowPlayer.getPlayer().equals(ePlayer.getPlayer()) && glowPlayer.getGlowVisibility().equals(GlowVisibility.OWN)))
						.forEach(glowPlayer -> {
							sendPacketNms(glowPlayer, packet);
						});
				break;
	...
	public void forceUpdateGlow(IEGlowPlayer ePlayer) {
		(ePlayer.getGlowTarget().equals(GlowTargetMode.ALL) ? Bukkit.getOnlinePlayers() : ePlayer.getCustomTargetList())
				.forEach(player -> {
